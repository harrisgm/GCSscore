...
In the vignette, we will go over some use cases for the GCSscore.  The data used here was obtained from the GEO database.  In the first example, we will investigate a set of ClariomS mouse arrays taken from GSE103380.  Further information regarding the experimental design can be found at: https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE103880
...
The compressed CEL files are directly directly from GEO using the \Rpackage{GEOquery} package.
<<results=hide, quiet=TRUE, message=FALSE>>=
...
#   (options set to prevent warning / errors)
