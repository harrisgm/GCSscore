...
##----------------------------------------------------------------------
makeAnnotPackageGCSs <- function(arraytype,
                             ...
                             quiet = FALSE, check = TRUE, build = TRUE, unlink = TRUE, ...)
{
  ...
  importRes <- do.call(importfun, c(arraytype = arraytype, pkgname = pkgname, chip.pd = chip.pd, pkg.db = pkg.db, list(...)))
